...
function mapDbRowToUiResult(row: AssessmentResultRow): AssessmentResult {
  return {
    ...
    aiRecommendation:
      row.ai_recommendation || "No AI recommendation available.",
...
export default function AssessmentPage() {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const filtered = ((data ?? []) as QuestionRow[]).filter((question) => {
        const domainName = Array.isArray(question.domains)
          ? question.domains[0]?.name || ""
          : question.domains?.name || "";

        return selectedDomains.includes(domainName);
      });

      const parsed: Question[] = filtered.map((question) => {
        ...
        const domainName = Array.isArray(question.domains)
          ? question.domains[0]?.name || null
          : question.domains?.name || null;
      ...
      console.error("Unexpected question fetch error:", error);
      setQuestions([]);
    } finally {
        ...
        <div className="relative space-y-6">
          <DialogHeader className="text-left">
            ...
            <DialogDescription className="text-slate-300">
              Pick up to 3 domains to shape the question set before the test
              starts.
            </DialogDescription>
          </DialogHeader>

          <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200">
            If you have existing result data, it stays on the page until this
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    selected
                      ? "border-cyan-300 bg-cyan-300/15 ring-1 ring-cyan-300/40"
                      : "border-white/10",
                  )}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-300">
                        {selected ? "Selected" : "Tap to add to your test"}
                      </p>
                    </div>
                    <div
                      className={cn(
                        ...
                        selected
                          ...
                          : "border-white/15 bg-white/5 text-slate-300 group-hover:border-cyan-300/50",
                      )}
                    >
                      {selected ? "✓" : "+"}
                    </div>
          ...
          <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200">
            ...
            <span className="text-slate-400">
              {selectedDomains.length ? selectedDomains.join(", ") : "None yet"}
            </span>
          </div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-primary/10 bg-linear-to-br from-slate-950 via-slate-900 to-indigo-950 px-6 py-8 text-white shadow-2xl shadow-slate-950/30">
            ...
            <div className="relative space-y-2">
              ...
              <p className="max-w-2xl text-sm text-slate-300">
                Review your latest assessment outcome or restart with a new
                domain selection.
              </p>
              <div className="flex flex-wrap gap-2 pt-2 text-xs text-slate-200/80">
                <Badge className="rounded-full bg-white/10 px-3 py-1 text-white hover:bg-white/15">
                  Completed
                </Badge>
                <Badge className="rounded-full bg-white/10 px-3 py-1 text-white hover:bg-white/15">
                  AI powered
            ...
            <CardContent className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {displayedResult.selectedDomains
                      .map((domain) => ({
                        domain,
                        score:
                          displayedResult.domainScores[domain]?.percentage || 0,
                      }))
                ...
                <div className="rounded-2xl border bg-muted/40 p-4">
                  ...
                  <p className="mt-2 text-lg font-semibold">
                    {displayedResult.totalScore} /{" "}
                    {displayedResult.totalQuestions}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {displayedResult.percentageScore}% overall
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-primary/10 bg-primary/5 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-foreground/90">
                  {displayedResult.aiRecommendation}
                </p>
              </div>
        ...
        <div className="space-y-6">
          <div className="relative overflow-hidden rounded-3xl border border-primary/10 bg-linear-to-br from-slate-950 via-slate-900 to-cyan-950 px-6 py-8 text-white shadow-2xl shadow-slate-950/30">
            ...
            <div className="relative space-y-3">
              ...
              <h1 className="text-3xl font-semibold tracking-tight">
                Freelancing Skill Assessment
              </h1>
              <p className="max-w-2xl text-sm text-slate-300">
                Select up to 3 domains and take a timed assessment to get a
                personalized freelancing direction.
              </p>
              <div className="flex flex-wrap gap-2 pt-2 text-xs text-slate-200/80">
                <Badge className="rounded-full bg-white/10 px-3 py-1 text-white hover:bg-white/15">
                  30 questions
          ...
          <Card className="border-border/60 shadow-lg shadow-slate-950/5">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm leading-6 text-muted-foreground">
                Use the popup to choose the domains you want to test. The dialog
                now follows the page theme so the entry flow feels cohesive.
              </p>
              ...
              {questions[currentQuestion]?.options.map((opt) => (
                <Label
                  ...
                  className={cn(
                    ...
                    "border-border/70 bg-muted/20 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-primary/5",
                  )}
                >
          ...
          <DialogContent className="overflow-hidden border-border/60 bg-linear-to-br from-slate-950 via-slate-900 to-indigo-950 text-white shadow-2xl shadow-slate-950/40 sm:max-w-2xl">
            ...
            <div className="relative space-y-6">
              <DialogHeader className="text-left">
                ...
                <DialogDescription className="text-slate-300">
                  Your answers have been scored and matched against the selected
                  domains.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {result?.selectedDomains
                      ...
                      .map((item, index) => (
                        <Badge
                          key={item.domain}
                          className="rounded-full bg-cyan-300/15 text-cyan-100 hover:bg-cyan-300/20"
                        >
                          {index + 1}. {item.domain}
                        </Badge>
                      ))}
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  <p className="text-sm text-slate-300">
                    {result?.percentageScore}% overall
                  </p>
                </div>
              </div>
